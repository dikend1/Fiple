...
final class RemoteFilesStore {
    ...
    private func ensureStore() -> (CloudKitRemoteFileStore, RemoteFileCache)? {
    ...
    func refresh() async {
        guard let (store, _) = ensureStore() else {
    ...
    func download(_ file: RemoteFile) async -> URL? {
        guard let (store, _) = ensureStore() else { return nil }
    ...
    func setPinned(_ file: RemoteFile, _ pinned: Bool) async -> Bool {
        guard let (_, cache) = ensureStore() else { return false }
        let ok: Bool
        if pinned {
