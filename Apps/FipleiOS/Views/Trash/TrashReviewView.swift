...
/// The Smart Trash review screen: a thumbnail grid of stale files found on the
/// Mac, multi-selected by tap (no swipes — the design's explicit choice), with
/// a bottom bar applying "Move to Trash" / "Keep" to the whole selection. Each
/// cell shows the file name, size, and a countdown to its auto-trash deadline.
struct TrashReviewView: View {
    ...
    private var actionBar: some View {
        ...
        .disabled(controller.trashActionInFlight)
        .padding(.horizontal, Theme.Spacing.lg)
...
/// One grid cell: QuickLook thumbnail (or a file-type placeholder while it
/// loads), selection ring + checkmark, file name, size, and deadline countdown.
    ...
    var body: some View {
        Button(action: onTap) {
            VStack(alignment: .leading, spacing: 6) {
                ...
                HStack(spacing: 4) {
                    Text(sizeText).font(.fiple(11)).foregroundStyle(Theme.Palette.secondary)
                    Spacer(minLength: 0)
                    Text(countdownText)
                        .font(.fiple(11, .medium))
